...
|------------|-------------|
| **LLM Extraction** | Optimised for Gemini 3 Flash with Gemini 2.5 Pro fallback (provider hooks for OpenAI/Anthropic exist for future use) |
...
Data is loaded from `11_execution_model.json` in the protocol output directory. This file is the intermediate extraction output generated by the execution model extractor during pipeline processing.

> **Note (v7.2):** Execution model data now exists in two locations:
> - **`protocol_usdm.json`** — Promoted native USDM entities (Condition, ScheduledDecisionInstance, Timing.windowLower/Upper, TransitionRule, Administration, etc.). This is the primary output.
> - **`11_execution_model.json`** — Intermediate extraction with full detail including extensions. Used by the Execution Model view in the UI.
