"""
LLM-Based Schema Fixer

Uses LLM to automatically fix USDM schema validation issues.
The fixer operates in batches, prioritizing high-impact fixes.

Architecture:
    1. Receive schema issues from OpenAPI validator
    2. Group issues by fixability and category
    3. Generate fix prompts with context
    4. Apply fixes to JSON data
    5. Re-validate and iterate if needed
    6. Return fixed data + remaining issues

Safety:
    - Never deletes existing data (only adds/modifies)
    - Backs up original data before modifications
...
try:
    ...
    HAS_LLM = False
    logger.warning("LLM utilities not available - schema fixes will be limited to programmatic fixes")
